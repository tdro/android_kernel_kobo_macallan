...
static int lp855x_read_byte(struct lp855x *lp, u8 reg, u8 *data)
{
	...
	if (lp->shutdown_complete) {
    	mutex_unlock(&lp->xfer_lock);
    	dev_err(lp->dev, "lp855x_read_byte after shutdown\n");
		return -ENODEV;
    }	
...
static int lp855x_write_byte(struct lp855x *lp, u8 reg, u8 data)
{
	...
	if (lp->shutdown_complete) {
        mutex_unlock(&lp->xfer_lock);
        dev_err(lp->dev, "lp855x_write_byte after shutdown\n");
	    return -ENODEV;
    }	
...
static void lp855x_shutdown(struct i2c_client *cl)
{
	struct lp855x *lp = i2c_get_clientdata(cl);
	mutex_lock(&lp->xfer_lock);
	lp->shutdown_complete = 1;
	mutex_unlock(&lp->xfer_lock);
    dev_err(lp->dev, "lp855x_shutdown..\n");
}
