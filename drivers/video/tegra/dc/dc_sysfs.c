...
extern int tegra_dsi_read_data(struct tegra_dc *dc,
				struct tegra_dc_dsi_data *dsi, u8 *pdata,
				u8 data_id,
				u16 data_len,
				u8 *read_data);
extern int tegra_dsi_write_data(struct tegra_dc *dc,
	...
	struct device_attribute *attr, char *buf)
{
	int i = 0;
#if 0
	u8 pdata[9][2] ={{0x02,0x00}, //max return length
					 {0x01,0x06}, //read 2 params
					 {0x01,0x0E}, //read 2 params
					 {0x01,0x10}, //read 2 params
					 {0x01,0x16}, //read 2 params
					 {0x02,0x06}, //read 2 params
					 {0x02,0x0E}, //read 2 params
					 {0x02,0x10}, //read 2 params
					 {0x02,0x16}, //read 2 params
#else
    u8 pdata[2][2] ={{0x02,0x00}, //max return length                                                                                                                                                                                        
                     {0x0A,0x00}, //read 2 params
#endif
	};
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		...
		(pdata[0][0] | (pdata[0][1] << 8)), buf);

	for (i = 1;i < ARRAY_SIZE(pdata); i++){
		tegra_dsi_read_data(dc, dc->out_data, NULL,
			dsi_command_dcs_read_with_no_params,
			(pdata[i][0] | (pdata[i][1] << 8)), buf);
	}
	tegra_dc_io_end(dc);
