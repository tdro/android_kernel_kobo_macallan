...
#define DSI_IDLE_OUT_WORK		0
#define DSI_RAM_BYPASS_MODE		1
...
static bool enable_read_debug = false;
...
static void tegra_dsi_start_dc_stream(struct tegra_dc *dc,
					struct tegra_dc_dsi_data *dsi);
static bool check_panel_status(struct tegra_dc *dc);

static struct tegra_dsi_cmd dcs_cmd_sleep_out = DSI_CMD_SHORT(DSI_DCS_WRITE_0_PARAM, DSI_DCS_EXIT_SLEEP_MODE, 0x0);
static struct tegra_dsi_cmd dcs_cmd_display_on = DSI_CMD_SHORT(DSI_DCS_WRITE_0_PARAM, DSI_DCS_SET_DISPLAY_ON, 0x0);
#if DSI_RAM_BYPASS_MODE
static struct tegra_dsi_cmd dsi_nop_cmd = DSI_CMD_SHORT(0x05, 0x0, 0x0);
static u8 sharp_fbuf_mode_sel[] = {0x10, 0x00, 0x17};
static struct tegra_dsi_cmd dcs_cmd_fbuf_mode_sel = DSI_CMD_LONG(DSI_GENERIC_LONG_WRITE, sharp_fbuf_mode_sel);
static u8 sharp_smode[] = {0x10, 0x07, 0x07};
static struct tegra_dsi_cmd dcs_cmd_smode = DSI_CMD_LONG(DSI_GENERIC_LONG_WRITE, sharp_smode);
#endif
#if DSI_IDLE_OUT_WORK
static struct tegra_dsi_cmd dcs_cmd_idle_out = DSI_CMD_SHORT(DSI_DCS_WRITE_0_PARAM, DSI_DCS_EXIT_IDLE_MODE, 0x0);
#endif
...
static int panel_regs_open(struct inode *inode, struct file *file)
{
	struct tegra_dc_dsi_data *dsi = tegra_dsi_instance[0];
	struct tegra_dc *dc = dsi->dc;

	printk( "%s\n", __func__ );

	check_panel_status(dc);
...
static const struct file_operations panel_regs_fops = {
	.open		= panel_regs_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
static void tegra_dc_dsi_debug_create(struct tegra_dc_dsi_data *dsi)
{
	...
	retval = debugfs_create_file("regs", S_IRUGO, dsidir, dsi,
		&dbg_fops);
	retval = debugfs_create_file("panel_regs", S_IRUGO, dsidir, dsi,
		&panel_regs_fops);
	if (!retval)
		goto free_out;
	return;
...
int tegra_dsi_read_data(struct tegra_dc *dc,
				struct tegra_dc_dsi_data *dsi, u8 *pdata,
				u8 data_id,
				u16 data_len,
				u8 *read_data)
{
	...
	err = _tegra_dsi_write_data(dsi, pdata,
		data_id,
		data_len);
	if (err < 0) {
		dev_err(&dc->ndev->dev,
...
static int tegra_dsi_enter_ulpm(struct tegra_dc_dsi_data *dsi)
{
	...
	/* 
	 * Hack: Trick DSI driver we are in ulpm.
	 */
	dsi->ulpm = true;
	return ret;
...
#if DSI_IDLE_OUT_WORK
static void dcs_idle_out_work_handler(struct work_struct *work);
static DECLARE_DELAYED_WORK(dcs_idle_out_work, dcs_idle_out_work_handler);
...
static void dcs_idle_out_work_handler(struct work_struct *work)
{
	...
	if (!dsi->enabled) {
		dev_err(&dc->ndev->dev, "%s: DSI controller suspended\r\n", __func__);
		return;
	}
	...
	err = tegra_dsi_write_data(dc, dsi,
						dcs_cmd_idle_out.pdata,
						dcs_cmd_idle_out.data_id,
						dcs_cmd_idle_out.sp_len_dly.data_len);
	if (err < 0) {
		dev_err(&dc->ndev->dev, "DSI idle_out write failed\n");
	}
...
static void _tegra_dc_dsi_enable(struct tegra_dc *dc)
{
	...
	/* Hack: serial to interlaced initialization */
...
//			tegra_dsi_send_dc_frames(dc, dsi, 2);
...
#if DSI_RAM_BYPASS_MODE
		err = tegra_dsi_write_data(dc, dsi,
							dcs_cmd_fbuf_mode_sel.pdata,
							dcs_cmd_fbuf_mode_sel.data_id,
							dcs_cmd_fbuf_mode_sel.sp_len_dly.data_len);
		if (err < 0) {
			dev_err(&dc->ndev->dev, "DSI fbuf_mode_sel write failed\n");
		}
		err = tegra_dsi_write_data(dc, dsi, 
							dsi_nop_cmd.pdata, 
							dsi_nop_cmd.data_id, 
							dsi_nop_cmd.sp_len_dly.data_len);
		if (err < 0) {
			dev_err(&dc->ndev->dev, "DSI nop write failed\n");
		}
		mdelay(3);
		...
		err = tegra_dsi_write_data(dc, dsi,
							dcs_cmd_smode.pdata,
							dcs_cmd_smode.data_id,
							dcs_cmd_smode.sp_len_dly.data_len);
		if (err < 0) {
			dev_err(&dc->ndev->dev, "DSI smode write failed\n");
		}
		err = tegra_dsi_write_data(dc, dsi, 
							dsi_nop_cmd.pdata, 
							dsi_nop_cmd.data_id, 
		 ...
		 */
		err = tegra_dsi_write_data_nosync(dc, dsi,
							dcs_cmd_sleep_out.pdata,
							dcs_cmd_sleep_out.data_id,
							dcs_cmd_sleep_out.sp_len_dly.data_len);
		if (err < 0) {
			dev_err(&dc->ndev->dev, "DSI link1_sleep_out write failed\n");
		}		
		tegra_dc_set_outdata(dc, tegra_dsi_instance[DSI_INSTANCE_1]);
		dsi = tegra_dc_get_outdata(dc);
		err = tegra_dsi_write_data_nosync(dc, dsi,
							...
							dcs_cmd_sleep_out.sp_len_dly.data_len);
		if (err < 0) {
			dev_err(&dc->ndev->dev, "DSI link2_sleep_out write failed\n");
		}
		tegra_dc_set_outdata(dc, tegra_dsi_instance[DSI_INSTANCE_0]);
		dsi = tegra_dc_get_outdata(dc);
		 ...
		 */
		err = tegra_dsi_write_data_nosync(dc, dsi,
							dcs_cmd_display_on.pdata,
							dcs_cmd_display_on.data_id,
							dcs_cmd_display_on.sp_len_dly.data_len);
		if (err < 0) {
			dev_err(&dc->ndev->dev, "DSI link1_display_on write failed\n");
		}

		mdelay(20);

		// issue idle out dcs command
#if DSI_IDLE_OUT_WORK
		cancel_delayed_work_sync(&dcs_idle_out_work);
		schedule_delayed_work(&dcs_idle_out_work, 0.5 * HZ);
#endif
				...
				struct tegra_dc_dsi_data *dsi)
{
	...
	/* Suspend DSI panel */
	if (tegra_dsi_instance[DSI_INSTANCE_1] != tegra_dc_get_outdata(dc)) {
		err = tegra_dsi_send_panel_cmd(dc, dsi,
...
static bool check_panel_status(struct tegra_dc *dc)
{
	u8 cmd_max_pkt_1byte[] = {0x01, 0x00};
	u8 cmd_max_pkt_2byte[] = {0x02, 0x00};
	u8 cmd_link1_error_report[] = {0x01, 0x20};
	u8 cmd_link2_error_report[] = {0x02, 0x20};
	u8 cmd_diagnostic_result[] = {0x0F, 0x00};
	u8 cmd_power_mode[] = {0x0A, 0x00};
	u8 cmd_fifo_status[] = {0x10, 0x33};
	/*
	u8 pdata_max_pkt_1byte[] = {0x01, 0x00};
	u8 pdata_fbuf_mode_sel[] = {0x10, 0x00};
	u8 pdata_mipi_if_sel[] = {0x10, 0x01};
	u8 pdata_mipi_mode_sel[] = {0x10, 0x02};
	u8 pdata_srstb_en[] = {0x10, 0x06};
	u8 pdata_smode[] = {0x10, 0x07};
	u8 pdata_e207h[] = {0xe2, 0x07};
	u8 pdata_e307h[] = {0xe2, 0x07};
	*/
	u8 buf[128];
	u8 diagnostic_result, power_mode, fifo_status;
	u16 err_report_link1, err_report_link2;

	tegra_dc_dsi_hold_host(dc);
	tegra_dc_io_start(dc);

	memset(buf, 0, sizeof(buf));
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_max_return_pkt_size,
		(cmd_max_pkt_2byte[0] | (cmd_max_pkt_2byte[1] << 8)), buf);

	// link1 error report
	memset(buf, 0, sizeof(buf));
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(cmd_link1_error_report[0] | (cmd_link1_error_report[1] << 8)), buf);
	err_report_link1 = (buf[5] << 8) | buf[6];

	// link2 error report
	memset(buf, 0, sizeof(buf));
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(cmd_link2_error_report[0] | (cmd_link2_error_report[1] << 8)), buf);
	err_report_link2 = (buf[5] << 8) | buf[6];//(buf[6] << 8) | buf[5];

	// diagnostic result
	memset(buf, 0, sizeof(buf));
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_dcs_read_with_no_params,
		(cmd_diagnostic_result[0] | (cmd_diagnostic_result[1] << 8)), buf);
	diagnostic_result = buf[5];

	// power_mode
	memset(buf, 0, sizeof(buf));
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_dcs_read_with_no_params,
		(cmd_power_mode[0] | (cmd_power_mode[1] << 8)), buf);
	power_mode = buf[5];
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_max_return_pkt_size,
		(cmd_max_pkt_1byte[0] | cmd_max_pkt_1byte[1] << 8), buf);
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(cmd_fifo_status[0] | cmd_fifo_status[1] << 8), buf);
	fifo_status = buf[5];
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_max_return_pkt_size,
		(pdata_max_pkt_1byte[0] | pdata_max_pkt_1byte[1] << 8), buf);
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(pdata_fbuf_mode_sel[0] | pdata_fbuf_mode_sel[1] << 8), buf);
	printk("FBUF_MODE_SEL = 0x%02x\r\n", buf[5]);
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(pdata_mipi_if_sel[0] | pdata_mipi_if_sel[1] << 8), buf);
	printk("MIPI_IF_SEL = 0x%02x\r\n", buf[5]);
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(pdata_mipi_mode_sel[0] | pdata_mipi_mode_sel[1] << 8), buf);
	printk("MIPI_MODE_SEL = 0x%02x\r\n", buf[5]);
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(pdata_srstb_en[0] | pdata_srstb_en[1] << 8), buf);
	printk("SRSTB_EN = 0x%02x\r\n", buf[5]);
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(pdata_smode[0] | pdata_smode[1] << 8), buf);
	printk("SMODE = 0x%02x\r\n", buf[5]);
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(pdata_e207h[0] | pdata_e207h[1] << 8), buf);
	printk("E207h = 0x%02x\r\n", buf[5]);
	...
	tegra_dsi_read_data(dc, dc->out_data, NULL,
		dsi_command_generic_read_request_with_2_param,
		(pdata_e307h[0] | pdata_e307h[1] << 8), buf);
	printk("E307h = 0x%02x\r\n", buf[5]);
*/
	...
	printk("%s: err_report_link1=0x%04x,err_report_link2=0x%04x,diagnostic_result=0x%02x,power_mode=0x%02x,fifo_status=0x%02x\r\n", __func__, 
		err_report_link1, err_report_link2, diagnostic_result, power_mode, fifo_status);

	/*
	 * In WKS, they don't use panel in production line. In this case all the status back from panel are zero 
	 * and we should return false to skip re-try.
	 */
	if (err_report_link1 == 0x0000 && err_report_link2 == 0x0000 &&
		diagnostic_result == 0x00 && power_mode == 0x00 && fifo_status == 0x00)
		return true;

	/*
	 * If any of the error detected, we retry initialization sequence
	 */
	if (err_report_link1 & 0xFFBF || err_report_link2 & 0xFFBD ||
		diagnostic_result != 0xC0 || power_mode != 0x07 || fifo_status != 0x00)
		return false;
...
#define MAX_RETRY	2
	...
	/* Retry if panel is not turned on successfully */
	msleep(200);
	while (check_panel_status(dc) != true && retry++ < MAX_RETRY) {
	
		printk("#############################################\r\n");
		printk("# Panel not turned on correctly, retry = %d #\r\n", retry);
		printk("#############################################\r\n");
