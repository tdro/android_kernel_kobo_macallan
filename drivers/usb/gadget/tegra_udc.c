...
static int __init tegra_udc_probe(struct platform_device *pdev)
{
	...
	if (pdata->psel_to_charger_gpio) {
		err = gpio_request(pdata->psel_to_charger_gpio, "PSEL_TO_CHARGER_GPIO");
		if (err) {
			dev_warn(&pdev->dev, "Failed to request gpio: %d\n", err);
		}else {
			err = gpio_direction_output(pdata->psel_to_charger_gpio, 1);
			if (err) {
				dev_warn(&pdev->dev, "Failed to get gpio as input: %d\n", err);
				gpio_free(pdata->psel_to_charger_gpio);
			}
			else
				udc->psel_to_charger_gpio = pdata->psel_to_charger_gpio;
		}
	}
