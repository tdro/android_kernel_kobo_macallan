...
{
	charger_type = chrg_type;
}
EXPORT_SYMBOL_GPL(external_power_status);
...
static int capacity_remap(int orig_capacity)	
{
#define RELATIVE_ZERO	5

	int batt_low_threshold = 0;
	struct odm_info oi;

	dora_get_odm_info(&oi);
	if (oi.pcb_ver >= 0 && oi.pcb_ver <= PCB_DVT1)
		batt_low_threshold = RELATIVE_ZERO;

	if (orig_capacity <= batt_low_threshold)
		return 0;
	else if (orig_capacity > batt_low_threshold && orig_capacity <= 100)
		return (orig_capacity - batt_low_threshold) * 100 / (100 - batt_low_threshold);
}
...
extern bool check_external_charger(void) ;
	...
	union power_supply_propval *val)
{
	...
	if ((di->chip == BQ27500) || (di->chip == BQ27510) || (di->chip == BQ27541)) {
		...
		else if (check_external_power() && check_external_charger())
...
static irqreturn_t bq27541_bat_low_irq(int irq, void *devid)
{
...
static int bq27x00_read_i2c(struct bq27x00_device_info *di, u8 reg, bool single)
{
	...
	mutex_lock(&di->sh);
	if (di->shutdown_complete == 1) {
		dev_err(di->dev, "bq27x00_read_i2c after shutdown\n");
		mutex_unlock(&di->sh);
		return -ENODEV;
	}
				...
				u16 val, bool single)
{
	...
	mutex_lock(&di->sh);
    if (di->shutdown_complete == 1) {
    	dev_err(di->dev, "bq27x00_write_i2c after shutdown\n");
    	mutex_unlock(&di->sh);
 		return -ENODEV;
    }
...
static unsigned int batterycapacity = 6550;
static ssize_t batterycapacity_cell_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	return snprintf(page, count, "%d\n", batterycapacity);
}

#define PROC_SYSINFO_DIR					"sysinfo"
#define PROC_SYSINFO_BATTERYCAPACITY_DIR	PROC_SYSINFO_DIR"/batterycapacity"
#define PROC_SYSINFO_BATTERYCAPACITY_CELL	"cell"
static void create_batterycapacity_cell_proc_file(void)
{
	struct proc_dir_entry *proc_file;
	struct proc_dir_entry *proc_batterycapacity_dir;
	
		
	proc_batterycapacity_dir = proc_mkdir(PROC_SYSINFO_BATTERYCAPACITY_DIR, NULL);
	if (!proc_batterycapacity_dir) {
		printk(KERN_ERR "[%s]-proc dir batterycapacity create failed!\n", __func__);
		return;
	}
	
	proc_file = create_proc_entry(PROC_SYSINFO_BATTERYCAPACITY_CELL, 0664, proc_batterycapacity_dir);
	if (proc_file) {
		proc_file->read_proc = batterycapacity_cell_read_proc;
	}
	else {
		printk(KERN_ERR "[%s]-proc file create failed!\n", __func__);
		return;
	}
}
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (di->chip != BQ27541 || (!pdata->bat_low_wakeup_to_shutdown))
		goto bat_low_wakeup_to_shutdown;
	retval = gpio_request(pdata->bat_low_wakeup_to_shutdown, "BAT_LOW_WAKEUP_SHUTDOWN_GPIO");
	if (retval) {
		dev_warn(&client->dev, "Failed to request gpio: %d\n", retval);
		goto bat_low_wakeup_to_shutdown;
	}
	retval = gpio_direction_input(pdata->bat_low_wakeup_to_shutdown);
	if (retval) {
		dev_warn(&client->dev, "Failed to get gpio as input: %d\n", retval);
		gpio_free(pdata->bat_low_wakeup_to_shutdown);
		goto bat_low_wakeup_to_shutdown;
	}
	irq = gpio_to_irq(pdata->bat_low_wakeup_to_shutdown);
	if (irq <= 0) {
		dev_warn(&client->dev, "Failed to get gpio as irq: %d\n", irq);
		gpio_free(pdata->bat_low_wakeup_to_shutdown);
		goto bat_low_wakeup_to_shutdown;
	}
	retval = request_irq(irq, bq27541_bat_low_irq,
...
static void bq27x00_battery_shutdown(struct i2c_client *client)
{
    struct bq27x00_device_info *di = i2c_get_clientdata(client);
    ...
    mutex_lock(&di->sh);
	di->shutdown_complete = 1;
    mutex_unlock(&di->sh);
    dev_err(&client->dev, "bq27x00_battery_shutdown..\n");
}
...
#endif
	.shutdown	= bq27x00_battery_shutdown,
	.id_table = bq27x00_id,
	.driver = {
		.name = "bq27541",
		.owner = THIS_MODULE,
