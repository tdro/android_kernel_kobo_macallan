...
static int bq2419x_init(struct bq2419x_chip *bq2419x)
{
	...
	if (stop_charging_flag == 1 && bq2419x->in_current_limit == 100) {
		ret = regmap_update_bits(bq2419x->regmap, BQ2419X_INPUT_SRC_REG,
				BQ2419x_CONFIG_MASK, val);
		if (ret < 0)
			dev_err(bq2419x->dev,
				"INPUT_SRC_REG update failed(stop_charging): %d\n", ret);
	}
	return ret;
}


static int bq2419x_get_battery_status(void)
{
...
static void bq2419x_charger_termination_config(struct bq2419x_chip *bq2419x, int charge_termination_enable)
{
	int ret = 0;
	unsigned int val = 0;

	ret = regmap_read(bq2419x->regmap, BQ2419X_CHRG_TERM_REG, &val);
	if (ret < 0)
		dev_err(bq2419x->dev, "error reading reg: 0x%x\n", BQ2419X_CHRG_TERM_REG);

	val &= ~0x0f;

	ret = regmap_write(bq2419x->regmap, BQ2419X_CHRG_TERM_REG, val);
	if (ret < 0)
...
static int bq2419x_charger_init(struct bq2419x_chip *bq2419x)
{
	...
	bq2419x_charger_termination_config(bq2419x, 0);
...
extern bool get_usb_status();
extern int otg_suspend_flag;
					...
					int min_uA, int max_uA)
{
	...
	if (otg_suspend_flag) {
		dev_info(bq_charger->dev, "otg_suspend_flag = %d \n", otg_suspend_flag);
		return 0;
	}
	
	if (stop_charging_flag) {
		dev_info(bq_charger->dev, "stop_charging_flag = %d\n",
				stop_charging_flag);
		return stop_charging_flag;
	}
	...
	if (max_uA == 0 && val != 0) {
		dev_info(bq_charger->dev, "%s - val = %d \n", __func__, val);
		return ret;
	}	
...
static int bq2419x_reset_wdt(struct bq2419x_chip *bq2419x, const char *from)
{
	int ret = 0;
	unsigned int reg01;
	int charge_termination_enable = 0;
	...
	/* Config charging termination */
	if ((bq2419x_get_battery_capacity() == 100 && 
	      bq2419x_get_battery_status() == POWER_SUPPLY_STATUS_FULL) ||(bq2419x_get_battery_capacity()  < 0))
	 	charge_termination_enable= 1;
	else
		charge_termination_enable = 0;

	bq2419x_charger_termination_config(bq2419x, charge_termination_enable);
scrub:
...
static int stop_charging_control(void)
{
	...
	if (stop_charging_flag == 1) {
		bq2419x_chip_data->in_current_limit = 100;
		ret = bq2419x_init(bq2419x_chip_data);
		if (ret < 0)
			goto error;
	}
	else if (stop_charging_flag == 0) {
		ret = regmap_read(bq2419x_chip_data->regmap, BQ2419X_SYS_STAT_REG, &val);
		if (ret < 0) {
			dev_err(bq2419x_chip_data->dev, "error reading reg: 0x%x\n",
					BQ2419X_SYS_STAT_REG);
			goto error;
		}
		
		if ((val & BQ2419x_VBUS_STAT) == BQ2419x_VBUS_UNKNOWN) {
			bq2419x_chip_data->status = 0;
			bq2419x_chip_data->usb_online = 0;
			bq2419x_chip_data->in_current_limit = 500;
			ret = bq2419x_init(bq2419x_chip_data);
			if (ret < 0)
			goto error;
			if (bq2419x_chip_data->update_status)
				bq2419x_chip_data->update_status
					(bq2419x_chip_data->status, 0);
		} else if ((val & BQ2419x_VBUS_STAT) == BQ2419x_VBUS_USB) {
			bq2419x_chip_data->status = 1;
			bq2419x_chip_data->usb_online = 1;
			bq2419x_chip_data->in_current_limit = 500;
			ret = bq2419x_init(bq2419x_chip_data);
			if (ret < 0)
				goto error;
			if (bq2419x_chip_data->update_status)
				bq2419x_chip_data->update_status
					(bq2419x_chip_data->status, 2);
		} else if ((val & BQ2419x_VBUS_STAT) == BQ2419x_VBUS_AC) {
			bq2419x_chip_data->status = 1;
			bq2419x_chip_data->ac_online = 1;
			bq2419x_chip_data->in_current_limit = 1500;
			ret = bq2419x_init(bq2419x_chip_data);
			if (ret < 0)
				goto error;
			if (bq2419x_chip_data->update_status)
...
error:
	dev_err(bq2419x_chip_data->dev, "Charger enable failed, err = %d\n", ret);
	return ret;	
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (bq2419x->suspended) {
		 mutex_unlock(&bq2419x->mutex);
		 printk(KERN_INFO "charging_stop_flag_store after shutdown\n");
		 return -ENODEV;
	 }
				...
				const struct i2c_device_id *id)
{
	...
	// Workaround for battery couldn't charge to full on DVT1.
	dora_get_odm_info(&oi);
	if (oi.pcb_ver >= 0 && oi.pcb_ver == PCB_DVT1){
		bq2419x->wdt_time_sec = 0;
		ret = regmap_read(bq2419x->regmap, BQ2419X_MISC_OPER_REG, &val);
		if (ret < 0) {
			dev_err(&client->dev, "error reading reg: 0x%x, err = %d\n",
						BQ2419X_MISC_OPER_REG, ret);
			return ret;
		}
		else {
			// Set Misc Operation Control Register REG07, Bit4(JEITA_VSET) to 1(4.2V)
			ret = regmap_write(bq2419x->regmap, BQ2419X_MISC_OPER_REG, (val |= 0x10));
			if (ret < 0) {
				dev_err(&client->dev, "error writing to reg: 0x%x\n",
						BQ2419X_MISC_OPER_REG);
				return ret;
			}
		}
	}
...
static int bq2419x_suspend(struct device *dev)
{
	...
	if ((bq2419x_get_battery_capacity() == 100 && 
	      bq2419x_get_battery_status() == POWER_SUPPLY_STATUS_FULL) ||(bq2419x_get_battery_capacity()  < 0)) {
	 	charge_termination_enable= 1;
		bq2419x->wdt_time_sec = 40;
		ret = bq2419x_watchdog_init(bq2419x, bq2419x->wdt_time_sec, "SUSPEND");
		if (ret < 0)
			dev_err(bq2419x->dev, "BQWDT init failed %d\n", ret);
	}
	else {
	...
	/* Configure charging current to 2000mA */
	ret = regmap_write(bq2419x->regmap, BQ2419X_INPUT_SRC_REG, 0x36);
	if (ret < 0) {
		dev_err(bq2419x->dev, "INPUT_SRC_REG write failed %d\n", ret);
		return ret;
	}
...
static int bq2419x_resume(struct device *dev)
{
	...
	if (check_external_power() != bq2419x->ac_online) {
		bq2419x->ac_online = check_external_power();
		power_supply_changed(&bq2419x->ac);
	}
