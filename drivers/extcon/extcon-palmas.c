...
extern bool dora_suspended;
extern void otgwl_hold_ext();
extern void otgwl_drop_ext();
static irqreturn_t palmas_extcon_irq(int irq, void *data)
{
	...
	// Workaround for can't wakeup system when plug-out AC.
	if (dora_suspended == true) {
		otgwl_hold_ext();
		otgwl_drop_ext();
	}
