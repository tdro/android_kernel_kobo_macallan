...
static void set_offcharging_mode_flag(int flag)
{
	...
	ret = palmas_write(palmas_dev, PALMAS_VALIDITY_BASE, PALMAS_BACKUP0, flag);
	if (ret < 0)
		dev_err(palmas_dev->dev, "Validity reg write failed\n");
	else {
		palmas_read(palmas_dev, PALMAS_VALIDITY_BASE, PALMAS_BACKUP0, &value);
		dev_info(palmas_dev->dev, "PALMAS_BACKUP0=0x%x \n", value);		
	}
}

static void unmask_int1_poweron_bit(void)
{
	int ret;
	unsigned int value;
	...
	palmas_read(palmas_dev, PALMAS_INTERRUPT_BASE,
					PALMAS_INT1_MASK, &value);
	dev_info(palmas_dev->dev, "INT1_MASK=0x%x \n", value);

	value &= ~PALMAS_INT1_MASK_PWRON;
	dev_info(palmas_dev->dev, "INT1_MASK=0x%x \n", value);
	ret = palmas_write(palmas_dev, PALMAS_INTERRUPT_BASE,
			PALMAS_INT1_MASK, value);
	if (ret < 0)
		dev_err(palmas_dev->dev, "Interrupt mask reg write failed\n");
	else {
		palmas_read(palmas_dev, PALMAS_INTERRUPT_BASE,
				PALMAS_INT1_MASK, &value);
		dev_info(palmas_dev->dev, "INT1_MASK=0x%x \n", value);
...
{
	int ret;
	unsigned int int3_lin_state;

	if (!palmas_dev)
		return 0;

	ret = palmas_read(palmas_dev, PALMAS_INTERRUPT_BASE,
				PALMAS_INT3_LINE_STATE, &int3_lin_state);
	if (ret < 0) {
		dev_err(palmas_dev->dev, "VBUS_STATUS read failed: %d\n", ret);
		return 0;
	}

	if (int3_lin_state & (1 << PALMAS_INT3_LINE_STATE_VBUS_SHIFT))
		return 1;
	else
		return 0;
}
EXPORT_SYMBOL_GPL(check_external_power);
...
static int vbus_irq = 0;
int check_vbus_interrupt(void)
{
	printk("%s - vbus_irq =%d \n", __func__, vbus_irq);
	if (vbus_irq == 1) {
		vbus_irq = 0;
...
EXPORT_SYMBOL(check_vbus_interrupt);
...
static irqreturn_t palmas_irq_thread(int irq, void *data)
{
	...
	for (i = 0; i < d->num_irqs; i++) {
		if (d->status_value[d->irqs[i].mask_reg_index] &
			...
			if (i == PALMAS_VBUS_IRQ) {
				dev_info(d->palmas->dev, "++++++++ PALMAS_VBUS_IRQ = %d +++++++\n", i);
				vbus_irq = 1;
			}
...
static void palmas_power_off(void)
{
	...
	if (oi.pcb_ver >= 0 && oi.pcb_ver <= PCB_EVT){
		dev_info(palmas_dev->dev, "EVT board-Power off ... \n");
		mask = 1;
		value = 0;
		goto _palmas_control_update;
	}
	...
	if (check_external_power()) {
		dev_info(palmas_dev->dev, "External power is connected, Rebooting \n");
		mask = 2;
		value = 2;
		goto _palmas_control_update;
	}
	else {		
		dev_info(palmas_dev->dev, "Power off ... \n");
		mask = 1;
		value = 0;
		goto _palmas_control_update;
	}
	
_palmas_control_update:
	palmas_control_update(palmas_dev, PALMAS_DEV_CTRL, mask, value);
	return;
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (ret) {
		dev_err(palmas->dev, "cannot register reboot notifier\n");
	}
