...
static struct odm_info odm_info;
...
static void __init macallan_gpio_init_configure(void)
{
	...
	switch (odm_info.pcb_ver) {
	case PCB_EVT:
		len = ARRAY_SIZE(init_gpio_mode_macallan_common_evt);
		pins_info = init_gpio_mode_macallan_common_evt;
		break;
	case PCB_DVT1:
		len = ARRAY_SIZE(init_gpio_mode_macallan_common_dvt1);
		pins_info = init_gpio_mode_macallan_common_dvt1;
		break;
	default:
...
int __init macallan_pinmux_init(void)
{
	...
	switch (odm_info.pcb_ver) {
	case PCB_EVT:
		tegra_pinmux_config_table(macallan_pinmux_common_evt,
					ARRAY_SIZE(macallan_pinmux_common_evt));
		break;
	case PCB_DVT1:
		tegra_pinmux_config_table(macallan_pinmux_common_dvt1,
					ARRAY_SIZE(macallan_pinmux_common_dvt1));
		break;
	default:
	...
	switch (odm_info.pcb_ver) {
	case PCB_EVT:
		tegra_pinmux_config_table(unused_pins_lowpower_evt,
					ARRAY_SIZE(unused_pins_lowpower_evt));
		break;
	case PCB_DVT1:
		tegra_pinmux_config_table(unused_pins_lowpower_dvt1,
					ARRAY_SIZE(unused_pins_lowpower_dvt1));
		break;
	default:
