...
struct bq2419x_charger_platform_data macallan_bq2419x_charger_pdata = {
	...
	.use_mains = 1,
#ifdef CONFIG_BATTERY_BQ27541
	.update_status = external_power_status,
#else
...
PALMAS_REGS_PDATA(smps123, 900,  1320, NULL, 0, 0, 0, 0,
...
static struct regulator_consumer_supply fixed_reg_vdd_lcd_hv_supply[] = {
...
FIXED_REG(2,	vdd_lcd_bl_en,	vdd_lcd_bl_en,
	NULL,	0,	1,
	TEGRA_GPIO_PH2,	false,	true,	1,	1800);
...
FIXED_REG(7,	vdd_lcd_hv,	vdd_lcd_hv,
	NULL,	0,	1,
	PALMAS_TEGRA_GPIO_BASE + PALMAS_GPIO7,	true,	true,	1,	1800);
...
int __init macallan_palmas_regulator_init(void)
{
	...
	if (odm_info.pcb_ver <= PCB_DVT1) {
		palmas_extcon_pdata.enable_id_pin_detection = false;
	}

	/* Starting from DVT2, SDIO clock source of WLAN changed to 
	 * GPIO_5 (AUDIO_CLK_32K) on TPS65913.
	 */
	if (odm_info.pcb_ver >= PCB_DVT2) {
		palmas_pdata.clk32k_init_data =  palmas_clk32k_idata;
		palmas_pdata.clk32k_init_data_size = ARRAY_SIZE(palmas_clk32k_idata);

		for (i = 0; i < ARRAY_SIZE(palmas_pincfg); i++) {
			if (palmas_pincfg[i].pin_name == PALMAS_PIN_NAME_GPIO5) {
				palmas_pincfg[i].pin_mux_option = PALMAS_PINMUX_CLK32KGAUDIO;
				break;
			}
		}
	}
...
extern int check_vbus_interrupt(void);
static void macallan_board_suspend(int lp_state, enum suspend_stage stg)
{
	if (stg == TEGRA_SUSPEND_BEFORE_CPU) {
		if (check_vbus_interrupt())
			pr_info("%s - Clear vbus irq\n", __func__);
	}
}
