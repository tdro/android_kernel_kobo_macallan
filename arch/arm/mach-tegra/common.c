...
static struct odm_info dora_odm_info;
...
static int __init parse_dora_odm_info(char *info)
{
	char *p = info;
	int size = 0;

	size = sizeof(dora_odm_info.bootloader_ver);
	strncpy(dora_odm_info.bootloader_ver, p, size - 1);
	p += size;

	size = sizeof(dora_odm_info.kernel_ver);
	strncpy(dora_odm_info.kernel_ver, DORA_KERNEL_VERSION, size - 1);

	size = sizeof(dora_odm_info.manufacturer);
	strncpy(dora_odm_info.manufacturer, DORA_MANUFACTURER, size - 1);
	
	dora_odm_info.pcb_ver = memparse(p, &p);
	dora_odm_info.ddr_sku = memparse(p+1, &p);
	return 1;
}

void dora_get_odm_info(struct odm_info *oi)
{
	memcpy(oi, &dora_odm_info, sizeof(struct odm_info));
}

__setup("odm_info=", parse_dora_odm_info);
